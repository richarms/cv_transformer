...
from attention.transformer import Transformer_Generation
from train_gen import train, test_evaluate
...
def execution(load_path=None, train_model=True, best=False, current_date_time=None, sm_variante='real'):
    ...
    print(output_dim)
    src_tokens = 43  # train_ds[0][0].shape[0]
    tgt_tokens = 64 - 43  # train_ds[0][0].shape[0]
    # print(input_dim, output_dim, tokens)

    model = Transformer_Generation(input_dim=input_dim, output_dim=output_dim, embed_dim=320, hidden_dim=2048, num_heads=8, layers=6, sm_variante=sm_variante, tgt_tokens=tgt_tokens, src_tokens=src_tokens)
