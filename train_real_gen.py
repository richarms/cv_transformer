    ...
    for epoch in range(epochs):
        ...
        for phase in ['train', 'valid']:
            ...
            for j, (data, label) in enumerate(dataloader):
                ...
                batch, tokens, features = data.shape
                data = data.type(torch.complex64)
                data = torch.reshape(torch.view_as_real(data), [batch, tokens, features * 2])

                x_data = data[:, :num_input].to(device)
                y_data = data[:, num_input:].to(device)
                y_label = label[:, num_input:].type(torch.float32).to(device)
                ...
                if phase == 'train':
                    ...
                    with torch.no_grad():
                        ...
                        trues[j * dataloader.batch_size: (j + 1) * dataloader.batch_size, :, :] = y_label.detach()
