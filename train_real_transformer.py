...
def train(model, train_dl, valid_dl, valid_ds, loss_fn, acc_fn, optimizer, scheduler, current_date_time, config, device="cuda", epochs=1):
    start = time.time()
    model.train()
    ...
    for epoch in range(epochs):
        ...
        for phase in ['train', 'valid']:
            ...
            for i, (x_data, y_data) in enumerate(dataloader):
                ...
                x_batch, x_tokens, x_features = x_data.shape
                x_data = torch.reshape(torch.view_as_real(x_data), [x_batch, x_tokens, x_features * 2])
            ...
            if phase == 'valid':
                ...
                full_save_path = os.path.join(config["model_path"], train_dict["model"].name, 'real_transformer', current_date_time, str(epoch))
...
def test_evaluate(model, test_dl, test_ds, loss_fn, acc_fn, current_date_time, config, device="cuda", best=False):
    model = model.to(device)
    test_loss = []
    test_acc = []
    ...
    if best is True:
        full_save_path = os.path.join(config["model_path"], model.name, 'real_transformer', current_date_time, 'test_results_best')
    else:
        full_save_path = os.path.join(config["model_path"], model.name, 'real_transformer', current_date_time, 'test_results')
    os.makedirs(full_save_path, exist_ok=True)
